...
class ControllerAnalyzer {
  ...
  async generateDocumentation(): Promise<void> {
    console.log('🚀 Generando documentación de API para IAs...');
  ...
  private extractEndpoints(content: string, basePath: string): EndpointInfo[] {
    ...
    while ((match = methodRegex.exec(content)) !== null) {
      ...
      // Extraer información del endpoint a partir de la posición del match
      const endpointInfo = this.extractEndpointDetails(content, match.index, method, fullPath);
      if (endpointInfo) {
  ...
  private extractEndpointDetails(content: string, startIndex: number, method: string, path: string): EndpointInfo | null {
    // Buscar desde la posición del decorador hasta el próximo método
    const nextMethodIndex = content.indexOf('@', startIndex + 1);
    const endIndex = nextMethodIndex === -1 ? content.length : nextMethodIndex;
    const methodSection = content.substring(startIndex, endIndex);

    // Extraer información de Swagger
    const summary = this.extractFromApiOperation(methodSection, 'summary') || 'Sin resumen';
    const description = this.extractFromApiOperation(methodSection, 'description') || 'Sin descripción';
    
    // Extraer autenticación y roles
    const auth = this.extractAuthInfo(methodSection);
    
    // Extraer parámetros
    const parameters = this.extractParameters(methodSection);
    
    // Extraer respuestas
    const responses = this.extractResponses(methodSection);
    
    // Extraer tags del endpoint (si difieren del controller)
    const endpointTags = this.extractEndpointTags(methodSection);
    ...
    return {
      ...
      parameters,
      responses,
      examples: this.extractExamples(methodSection),
    };
  }

  private extractFromApiOperation(content: string, field: string): string | null {
    // Buscar patrones más flexibles para @ApiOperation
    const patterns = [
    ...
    // Si no encuentra el patrón específico, buscar en todo el bloque @ApiOperation
    const operationMatch = content.match(/@ApiOperation\(\s*\{([^}]+)\}\s*\)/s);
    if (operationMatch) {
  ...
  private extractApiParams(content: string): Array<{ name: string; description: string; example?: string; required: boolean }> {
    ...
    // Buscar tanto patrones de una línea como multilínea
    const patterns = [
      /@ApiParam\(\s*\{([^}]+)\}\s*\)/g,
      /@ApiParam\(\s*\{([^}]*(?:\n[^}]*)*)\}\s*\)/g,
    ];
