...
import { OpenChatViewCommandHandler } from '../open-chat-view.command-handler';
...
describe('OpenChatViewCommandHandler', () => {
  let handler: OpenChatViewCommandHandler;
  ...
  beforeEach(async () => {
    ...
    handler = module.get<OpenChatViewCommandHandler>(
      OpenChatViewCommandHandler,
    );
  });
  ...
  describe('execute', () => {
    it('deberia abrir vista del chat exitosamente para un visitante', async () => {
      // Arrange
      const mockChat = createMockChat(mockVisitorId);
      mockChatRepository.findById.mockResolvedValue(ok(mockChat));

      const command = new OpenChatViewCommand(mockChatId, mockVisitorId, [
        'visitor',
      ]);
      ...
      expect(mockEventBus.publish).toHaveBeenCalledWith(
        expect.any(ChatViewOpenedEvent),
      );
    });

    it('deberia abrir vista del chat exitosamente para un comercial', async () => {
      // Arrange
      const mockChat = createMockChat(mockVisitorId);
      mockChatRepository.findById.mockResolvedValue(ok(mockChat));

      const command = new OpenChatViewCommand(mockChatId, mockCommercialId, [
        'commercial',
      ]);
    ...
    it('deberia usar el timestamp proporcionado', async () => {
      ...
      expect(mockEventBus.publish).toHaveBeenCalledWith(
        expect.objectContaining({
          attributes: expect.objectContaining({
            view: expect.objectContaining({
              openedAt: new Date(customTimestamp),
            }),
          }),
        }),
      );
