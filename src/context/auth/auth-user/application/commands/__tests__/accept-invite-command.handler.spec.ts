...
import { USER_PASSWORD_HASHER } from '../../service/user-password-hasher';
import { ok, err } from 'src/context/shared/domain/result';
import { InvitePersistenceError } from '../../../domain/errors/invite.errors';
...
describe('AcceptInviteCommandHandler', () => {
  ...
  beforeEach(async () => {
    ...
    handler = module.get<AcceptInviteCommandHandler>(
      AcceptInviteCommandHandler,
    );
  });
  ...
  it('debe lanzar error cuando el repositorio de invitaciones falla', async () => {
    ...
    mockInviteRepository.match.mockResolvedValue(
      err(new InvitePersistenceError('Database error')),
    );
  ...
  it('debe integrar correctamente el contexto de eventos', async () => {
    ...
    expect(mockUserWithContext.updatePassword).toHaveBeenCalledWith(
      'hashedPassword',
    );
