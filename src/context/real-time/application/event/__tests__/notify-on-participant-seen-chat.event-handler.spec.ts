...
describe('NotifyOnParticipantSeenChatEventHandler', () => {
  ...
  beforeEach(async () => {
    ...
    handler = module.get<NotifyOnParticipantSeenChatEventHandler>(
      NotifyOnParticipantSeenChatEventHandler,
    );
  ...
  describe('handle', () => {
    it('debe enviar notificaciones a todos los participantes excepto al que vio el chat', async () => {
      ...
      // Assert: Verificar que se llamó al servicio de notificaciones para cada participante excepto el que vio el chat
    ...
    it('no debe enviar notificaciones si no hay más participantes en el chat', async () => {
      ...
      expect(loggerWarnSpy).toHaveBeenCalledWith(
        expect.stringContaining('No recipients found for participant seen chat event with ID'),
      );
    });
  });
});