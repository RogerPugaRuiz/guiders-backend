...
import { UpdateVisitorNameCommandHandler } from '../update-visitor-name-command.handler';
...
describe('UpdateVisitorNameCommandHandler', () => {
  let handler: UpdateVisitorNameCommandHandler;
  ...
  beforeEach(async () => {
    ...
    handler = module.get<UpdateVisitorNameCommandHandler>(
      UpdateVisitorNameCommandHandler,
    );
  });

  it('debe actualizar correctamente el nombre del visitante', async () => {
    // Preparar los mocks
    ...
    // Ejecutar el comando
    const command = new UpdateVisitorNameCommand(mockVisitorId, 'New Name');
